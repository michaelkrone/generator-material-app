 ...
 */
gulp.task('junit:server', function (cb) {
	return tests.testFn(conf.src.server.unitTests, 'mocha-jenkins-reporter', cb);
});
 ...
 * Run unit tests
 */
gulp.task('junit:client', function (cb) {
	return tests.testFn(conf.src.client.unitTests, 'mocha-jenkins-reporter', cb);
});
 ...
 */
gulp.task('analyze:server', function (cb) {
	...
	return plato.inspect(conf.src.server.js, outDir, options, function () {
		cb();
	});
});
 ...
 */
gulp.task('analyze:client', function (cb) {
	var outDir = path.join(conf.dirs.build, conf.dirs.plato, 'client');

	return plato.inspect(conf.src.client.js, outDir, conf.options.plato, function () {
		cb();
	});
});
